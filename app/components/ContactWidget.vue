<template>
  <div
    ...
    :class="{ 'contact-widget--open': isOpen }"
    :style="{ '--items-last-index': contactItems.length - 1 }"
  >
...
const SOCIAL_LINKS = {
  instagram: 'https://www.instagram.com/mirtu.kz',
  tiktok: 'https://www.tiktok.com/@mirtu252',
  youtube: 'https://www.youtube.com/@Mirtu_kz',
}
...
const ICONS = {
  ...
  tiktok: `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M16.6 5.82C17.5 7.3 18.9 8.36 20.5 8.72V12.1C19.1 12.1 17.8 11.7 16.7 11V16.2C16.7 19.03 14.43 21.3 11.6 21.3C8.77 21.3 6.5 19.03 6.5 16.2C6.5 13.37 8.77 11.1 11.6 11.1C12 11.1 12.39 11.15 12.76 11.24V14.5C12.4 14.38 12.01 14.34 11.62 14.4C10.55 14.57 9.7 15.5 9.7 16.62C9.7 17.84 10.68 18.82 11.9 18.82C13.03 18.82 13.9 17.95 13.9 16.82V5.82H16.6Z" fill="currentColor"/></svg>`,
  youtube: `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M21.58 7.2C21.32 6.34 20.66 5.66 19.82 5.4C18.28 5 12 5 12 5C12 5 5.72 5 4.18 5.4C3.34 5.66 2.68 6.34 2.42 7.2C2 8.76 2 12 2 12C2 12 2 15.24 2.42 16.8C2.68 17.66 3.34 18.34 4.18 18.6C5.72 19 12 19 12 19C12 19 18.28 19 19.82 18.6C20.66 18.34 21.32 17.66 21.58 16.8C22 15.24 22 12 22 12C22 12 22 8.76 21.58 7.2Z" stroke="currentColor" stroke-width="1.6" stroke-linejoin="round"/><path d="M10 15.5V8.5L16 12L10 15.5Z" fill="currentColor"/></svg>`,
}
...
const contactItems = computed(() => {
  ...
  const whatsappHref = buildWhatsappLink(props.dataContacts?.whatsapp_contact)
  const telegramHref = buildTelegramLink(props.dataContacts?.telegram_contact)

  if (whatsappHref) {
    items.push({ id: 'whatsapp', href: whatsappHref, label: 'WhatsApp', icon: ICONS.whatsapp, external: true })
  }
  if (telegramHref) {
    items.push({ id: 'telegram', href: telegramHref, label: 'Telegram', icon: ICONS.telegram, external: true })
  }

  items.push(
    { id: 'instagram', href: SOCIAL_LINKS.instagram, label: 'Instagram', icon: ICONS.instagram, external: true },
    { id: 'tiktok', href: SOCIAL_LINKS.tiktok, label: 'TikTok', icon: ICONS.tiktok, external: true },
    { id: 'youtube', href: SOCIAL_LINKS.youtube, label: 'YouTube', icon: ICONS.youtube, external: true },
  )
